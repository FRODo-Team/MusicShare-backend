...
public:
    ...
    virtual SongResponseDTO GetById(uint32_t id) = 0 const;

    virtual std::vector<SongResponseDTO> GetByTitle(const std::string& title) = 0 const;

    virtual std::vector<SongResponseDTO> GetByArtist(const std::string& artist) = 0 const;
