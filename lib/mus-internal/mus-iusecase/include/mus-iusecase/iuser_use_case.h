...
public:
    ...
    virtual UserResponseDTO GetByUsername(const std::string& username) = 0 const;

    virtual std::vector<UserResponseDTO> GetByNicknames(const std::vector<std::string>& nicknames) = 0 const;

    virtual UserResponseDTO GetById(uint32_t id) = 0 const;
