...
public:
    ...
    virtual std::vector<PlaylistResponseDTO> GetByUserId(uint32_t user_id) = 0 const;

    virtual PlaylistResponseDTO GetById(uint32_t id) = 0 const;

    virtual std::vector<uint32_t> GetSongs(uint32_t playlist_id) = 0 const;
